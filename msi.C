...
class mu_1_h_state: public mu__byte
{
 public:
  ...
  mu_1_h_state (char *name, int os): mu__byte(7, 12, 3, name, os) {};
  mu_1_h_state (void): mu__byte(7, 12, 3) {};
  mu_1_h_state (int val): mu__byte(7, 12, 3, "Parameter or function result.", 0)
  {
     operator=(val);
  };
...
char *mu_1_h_state::values[] = {"H_I","H_S","H_M","H_MSD","H_MMD","H_SMA",NULL };
...
class mu_1_p_state: public mu__byte
{
 public:
  ...
  {
    if (val.defined())
      return ( s << mu_1_p_state::values[ int(val) - 13] );
    else return ( s << "Undefined" );
  };

  mu_1_p_state (char *name, int os): mu__byte(13, 23, 4, name, os) {};
  mu_1_p_state (void): mu__byte(13, 23, 4) {};
  mu_1_p_state (int val): mu__byte(13, 23, 4, "Parameter or function result.", 0)
  {
     operator=(val);
  };
...
char *mu_1_p_state::values[] = {"P_I","P_S","P_M","P_ISD","P_IMAD","P_IMA","P_SMAD","P_SMA","P_MIA","P_SIA","P_IIA",NULL };
...
class mu_1_MessageType: public mu__byte
{
 public:
  ...
  {
    if (val.defined())
      return ( s << mu_1_MessageType::values[ int(val) - 24] );
    else return ( s << "Undefined" );
  };

  mu_1_MessageType (char *name, int os): mu__byte(24, 35, 4, name, os) {};
  mu_1_MessageType (void): mu__byte(24, 35, 4) {};
  mu_1_MessageType (int val): mu__byte(24, 35, 4, "Parameter or function result.", 0)
  {
     operator=(val);
  };
...
char *mu_1_MessageType::values[] = {"GetM","GetMAck","GetS","GetSAck","FwdGetM","FwdGetS","Inv","InvAck","PutM","PutS","PutAck","InvAckAll",NULL };
...
const int mu_H_S = 8;
const int mu_H_M = 9;
const int mu_H_MSD = 10;
const int mu_H_MMD = 11;
const int mu_H_SMA = 12;
const int mu_P_I = 13;
const int mu_P_S = 14;
const int mu_P_M = 15;
const int mu_P_ISD = 16;
const int mu_P_IMAD = 17;
const int mu_P_IMA = 18;
const int mu_P_SMAD = 19;
const int mu_P_SMA = 20;
const int mu_P_MIA = 21;
const int mu_P_SIA = 22;
const int mu_P_IIA = 23;
const int mu_GetM = 24;
const int mu_GetMAck = 25;
const int mu_GetS = 26;
const int mu_GetSAck = 27;
const int mu_FwdGetM = 28;
const int mu_FwdGetS = 29;
const int mu_Inv = 30;
const int mu_InvAck = 31;
const int mu_PutM = 32;
const int mu_PutS = 33;
const int mu_PutAck = 34;
const int mu_InvAckAll = 35;
...
bool mu__boolexpr18;
  if ((mu_msg.mu_mtype) == (mu_InvAck)) mu__boolexpr18 = TRUE ;
  else {
bool mu__boolexpr19;
  if (!((mu_msg.mu_mtype) == (mu_GetMAck))) mu__boolexpr19 = FALSE ;
  else {
...
for(int mu_n = 1; mu_n <= 6; mu_n++)
  if (( ( mu_n >= 1 ) && ( mu_n <= 3 ) )|| ( ( mu_n >= 6 ) && ( mu_n <= 6 ) )) {
bool mu__boolexpr21;
  if (!((mu_n>=1 && mu_n<=3))) mu__boolexpr21 = FALSE ;
  else {
  ...
  for (mu_i = 0; ; mu_i=mu_i+1)
    {
      if (mu_HomeNode.mu_sharers.valid[(int)mu_i].value())
        {
          if ( (mu_HomeNode.mu_sharers[mu_i]) == (mu_n) ) mu__intexpr22++;
        }
      if (mu_i == 3-1) break;
    }
  }
  ...
  for (mu_i = 0; ; mu_i=mu_i+1)
    {
      if (mu_sharers_copy.valid[(int)mu_i].value())
        {
          if ( (mu_sharers_copy[mu_i]) == (mu_n) ) mu__intexpr23++;
        }
      if (mu_i == 3-1) break;
    }
  }
...
/*** end multisetremove 1 declaration ***/
  mu_1__type_6_id mu__idexpr24;
  mu_1__type_6_id mu_i;
  for (mu_i = 0; ; mu_i=mu_i+1)
    {
      if (mu_sharers_copy.valid[(int)mu_i].value())
        {
          if ( (mu_sharers_copy[mu_i]) == (mu_n) ) { mu__idexpr24 = mu_i; mu_sharers_copy.multisetremove(mu__idexpr24); };
        }
      if (mu_i == 3-1) break;
    }
/*** end multisetremove 1 declaration ***/
  ...
  for (mu_i = 0; ; mu_i=mu_i+1)
    {
      if (mu_HomeNode.mu_sharers.valid[(int)mu_i].value())
        {
          if ( (mu_HomeNode.mu_sharers[mu_i]) == (mu_msg.mu_src) ) mu__intexpr26++;
        }
      if (mu_i == 3-1) break;
    }
  }
/*** end multisetcount 11 declaration ***/
mu_is_source = mu__intexpr26;
...
{
mu_MultiSetCount_H_PutS ( mu_H_I, mu_H_S );
...
break;
case mu_P_ISD:
if ( (mu_msg.mu_mtype) == (mu_Inv) )
{
mu_msg_processed = mu_false;
}
else
{
if ( (mu_msg.mu_mtype) == (mu_GetSAck) )
{
mu_pv = mu_msg.mu_val;
mu_ps = mu_P_S;
}
...
case mu_P_IMAD:
bool mu__boolexpr27;
  if ((mu_msg.mu_mtype) == (mu_FwdGetS)) mu__boolexpr27 = TRUE ;
  else {
  mu__boolexpr27 = ((mu_msg.mu_mtype) == (mu_FwdGetM)) ; 
}
if ( mu__boolexpr27 )
{
mu_msg_processed = mu_false;
...
case mu_P_IMA:
bool mu__boolexpr28;
  if ((mu_msg.mu_mtype) == (mu_FwdGetS)) mu__boolexpr28 = TRUE ;
  else {
  mu__boolexpr28 = ((mu_msg.mu_mtype) == (mu_FwdGetM)) ; 
}
if ( mu__boolexpr28 )
{
mu_msg_processed = mu_false;
...
case mu_P_SMAD:
bool mu__boolexpr29;
  if ((mu_msg.mu_mtype) == (mu_FwdGetS)) mu__boolexpr29 = TRUE ;
  else {
  mu__boolexpr29 = ((mu_msg.mu_mtype) == (mu_FwdGetM)) ; 
}
if ( mu__boolexpr29 )
{
mu_msg_processed = mu_false;
...
case mu_P_SMA:
bool mu__boolexpr30;
  if ((mu_msg.mu_mtype) == (mu_FwdGetS)) mu__boolexpr30 = TRUE ;
  else {
  mu__boolexpr30 = ((mu_msg.mu_mtype) == (mu_FwdGetM)) ; 
}
if ( mu__boolexpr30 )
{
mu_msg_processed = mu_false;
...
bool Condition(unsigned r)
{
  ...
  if (r>=31 && r<=36) return R3.Condition(r-31);
  if (r>=37 && r<=39) return R4.Condition(r-37);
  if (r>=40 && r<=42) return R5.Condition(r-40);
...
void Code(unsigned r)
{
  ...
  if (r>=31 && r<=36) { R3.Code(r-31); return; } 
  if (r>=37 && r<=39) { R4.Code(r-37); return; } 
  if (r>=40 && r<=42) { R5.Code(r-40); return; } 
...
int Priority(unsigned short r)
{
  ...
  if (r>=31 && r<=36) { return R3.Priority(); } 
  if (r>=37 && r<=39) { return R4.Priority(); } 
  if (r>=40 && r<=42) { return R5.Priority(); } 
...
char * Name(unsigned r)
{
  ...
  if (r>=31 && r<=36) return R3.Name(r-31);
  if (r>=37 && r<=39) return R4.Name(r-37);
  if (r>=40 && r<=42) return R5.Name(r-40);
  if (r>=43 && r<=45) return R6.Name(r-43);
  if (r>=46 && r<=48) return R7.Name(r-46);
  return NULL;
}
};
const unsigned numrules = 49;

/********************
  parameter
 ********************/
#define RULES_IN_WORLD 49
...
int mu__invariant_45() // Invariant "modified implies empty sharers list"
{
bool mu__boolexpr46;
  if (!((mu_HomeNode.mu_state) == (mu_H_M))) mu__boolexpr46 = TRUE ;
  else {
...
/*** end multisetcount 12 declaration ***/
  mu__boolexpr46 = ((mu__intexpr47) == (0)) ; 
}
return mu__boolexpr46;
};

bool mu__condition_48() // Condition for Rule "modified implies empty sharers list"
{
...
int mu__invariant_49() // Invariant "values in valid state match last write"
{
bool mu__quant50; 
mu__quant50 = TRUE;
{
...
bool mu__boolexpr51;
  ...
  mu__boolexpr51 = ((mu_Procs[mu_n].mu_val) == (mu_LastWrite)) ; 
}
if ( !(mu__boolexpr51) )
...
{"modified implies empty sharers list", &mu__condition_48, NULL, FALSE},
